...
| `Author_1_Gender`...`Author_9_Gender` | Gender code for the corresponding author: `m` (male), `f` (female), `u` (unknown/undetermined), or blank |
...
- Author gender codes are manually assigned first-name judgment calls, not yet validated against a systematic API.
- `Primary_Method` is not yet populated.
- Book prizes and student-paper awards (e.g. ASA Ronald Burt Outstanding Student Paper Award) are intentionally excluded — this dataset covers article prizes for established scholars only.
